...
hyperparams = pd.concat([
    pd.read_csv(f, index_col="version") for f in glob.glob(os.path.join(hparams_dir, "hparams_2371636.csv"))
])

data = pd.concat((metrics, hyperparams), join="inner", axis=1)
data.to_csv(os.path.join(out_path, "metrics_dense_deep_p100.csv"))
