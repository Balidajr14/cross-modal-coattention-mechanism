...
class CoAttention_Model(pl.LightningModule):
    def __init__(self, hparams: Namespace):
        ...
        self.Image_Encoder = DenseNet3d(growth_rate=self.hparams.growth_rate,
                                        dropout_p=self.hparams.dropout)
        self.Emr_Encoder = Emr_Encoder(feature_size=self.hparams.feature_size, 
                                       embedding_size=self.Image_Encoder.out_channels,
                                       num_units=self.hparams.block_shape,
                                       dropout=self.hparams.dropout_emr)
        
        self.Multihead_Attention = MultiheadCoattention(embedding_dim=self.Image_Encoder.out_channels,
                                                        num_units=self.hparams.block_shape,
    ...
    def forward(self, x):
        ...
        emr = F.relu(self.Emr_Encoder(emr))
        y = F.relu(self.Image_Encoder(x_hires))
        ...
        y = y.permute(0, 2, 1)
        print(y.size())
        z, self.weights = self.Multihead_Attention(queries=emr,
                                                   keys=y,
                                                   values=y)
    ...
    def add_model_specific_args(parent_parser: ArgumentParser):
        ...
        parser.add_argument('--feature_size', 
                            type=int, 
                            default=484)
        parser.add_argument('--dropout_emr', 
                            type=float, 
                            default=0.)
        return parser
'''
